...
由于现在接收完全是单线程的（一个线程监听一个套接字），所以目前没有给去重方法提供线程安全保证。

### 现有功能和下一阶段

第一阶段功能：在两台部署了程序的主机之间的所有连接上广播数据包。
...
- 一跳转发：程序将来自直连主机的转发包转发到另一台直连主机。
  这个功能不需要路由选择，但只适用于简单的星型网络。
- 广播转发：对非直连的目标地址，将转发包发到除来源之外的所有直连主机。
  这个功能实现容易，但性能较差而且可能引发广播风暴。
- 自动路由选择：模仿物理网络中的第三层协议，直连主机之间通过交换路由表获得对整个虚拟网络的认识以实现虚拟网络上的最优路由。
- 线程池 + 内存池：线程池用于提高 CPU 利用率，内存池用于在多线程中实现**零复制转发**。对于一个网络程序，零复制是最重要的！
